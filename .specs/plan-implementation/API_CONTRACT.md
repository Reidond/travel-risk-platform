...
- Vite + React 19 + TypeScript (strict), react-router, @tanstack/react-query, react-i18next (uk default + en, language switcher in header, ALL strings via i18n incl. term/risk labels), Recharts for charts. UI kit: shadcn/ui (new-york, Radix primitives, CSS variables) on Tailwind CSS v4 — light-only theme whose tokens port the original palette (`--primary` = #1565c0 as oklch, page bg #f5f6f8, text #1f2430); sonner for success toasts (errors stay inline alerts); accessible (labels, keyboard nav, focus rings, WCAG AA contrast). Custom shared components (`Modal` on Dialog, `ConfirmDialog` on AlertDialog, `Pagination`, `EmptyState`, badges) live in `src/components/`; generated primitives in `src/components/ui/`.
- Risk color scale (used consistently): R1 `#c62828`, R2 `#ef6c00`, R3 `#f9a825`, R4 `#9ccc65`, R5 `#2e7d32`; r* terms L→H use green→red (L `#2e7d32`, BA `#9ccc65`, A `#f9a825`, AA `#ef6c00`, H `#c62828`). These hex values are contractual, live only in `src/lib/colors.ts`, and are applied inline (never via theme tokens).
- Pages (per plan §6):
