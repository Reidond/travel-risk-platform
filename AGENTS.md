...
- **CI/CD**: `.github/workflows/ci.yml` runs every gate as its own parallel job (ruff, pytest×2, eslint, tsc+vite build, i18n parity, wrangler validate) on PRs; `deploy.yml` re-runs CI then deploys to Cloudflare on pushes to `main` (secrets: `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`); `publish-pypi.yml` re-runs CI then publishes `core/` to PyPI via Trusted Publishing (OIDC, no token secret) on two channels — stable on `core-v*` tags (tag must match `core/pyproject.toml`'s version; GitHub Release on the tag) and preview on every push to `main` (derived version `next-patch.dev<commit count>`, never committed; auto-tagged `core-dev-v<version>-<sha>` with a GitHub prerelease). Consumers get previews only via `pip install --pre` / `uv add --prerelease allow`. Cut a stable release with `uv run scripts/release_core.py [--bump patch|minor|major | <version>]` (validates clean/up-to-date main, optionally bumps + commits, runs the build/import gate, then pushes the `core-v*` tag CI publishes from; `--dry-run` to preview). Actions are SHA-pinned; Dependabot bumps them weekly.
