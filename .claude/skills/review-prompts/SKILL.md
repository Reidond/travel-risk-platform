...
argument-hint: "[scope, e.g. 'risk-assessment', 'alerts', 'all']"
...
- `$ARGUMENTS` — Optional scope filter:
  - A category path: `{feature}` (e.g. `risk-assessment`, `alerts`)
  - A sub-path: `{feature}/{sub-feature}` (e.g. `risk-assessment/itinerary`)
  - A specific file: `prompts/{feature}/{template}.txt`
