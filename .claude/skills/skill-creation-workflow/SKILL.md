...
and invoke the skill-creator. Since `skill-creator` is `disable-model-invocation: true`,
you cannot invoke it via the Skill tool — instead, read its SKILL.md and follow its
...
Run the `skill-reviewer` checklist against the newly created skill. Since
`skill-reviewer` has `disable-model-invocation: true`, read its SKILL.md and
follow its Review Mode directly.
