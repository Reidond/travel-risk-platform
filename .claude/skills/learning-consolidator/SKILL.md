...
- `--category <name>` — Process only entries in the specified category (e.g., `pitfall`, `architecture`).
...
4. If the rule also needs a code example, check if a relevant reference skill covers it. Add an example there only if the pattern is non-obvious.
